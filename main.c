...
void generate_pagination(Post *posts, int total_posts, int posts_per_page) {
	...
	if(header == NULL ) {
		fprintf(archive, "<header><h1>daisy</h1></header>");
	}
	else if (header != NULL) {
...
void generate(void) {
	...
	while (fgets(txt, MAX_LENGTH, fptr) != NULL) {
		...
		if (len == 0) {
			...
			else if (len > 2 && txt[0] == '`' && txt[1] == '`' && txt[2] == 'a' && (i == 0 || txt[i-1] != '\\')) {
				if (mdt.blockcode == 0) {
			...
			} else if (len > 2 && txt[0] == '`' && txt[1] == '`' && txt[2] == 'b' && (i == 0 || txt[i-1] != '\\')) {
				if (mdt.blockcode == 1) {
